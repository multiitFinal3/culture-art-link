	...
	public void signUpPost(
		...
		try {
			...
			if (uploadFile != null) {
				...
				String fileUUIDName = UUID.randomUUID() + "_" + uploadFile.getOriginalFilename();
...
//				attachment = attachment.substring(startIndex);
				...
				String fileUUIDName = UUID.randomUUID() + "_" + file.getOriginalFilename();
				attachment = "/user/userProfile/" + fileUUIDName.trim();
				// 네이버 스토리지 사용
...
//
	...
	public String insertUserBigLoveHateKeyword(@AuthenticationPrincipal VWUserRoleDTO user, @RequestParam(name = "performanceKeyword", required = false) String performanceKeyword, @RequestParam(name = "exhibitionKeyword", required = false) String exhibitionKeyword, @RequestParam(name = "festivalKeyword", required = false) String festivalKeyword, @RequestParam(name = "culturalPropertiesKeyword", required = false) String culturalPropertiesKeyword, @RequestParam(name = "loveOrHate", required = true) String loveOrHate) {
		...
		if (loveOrHate.equals("L")) {
			...
			performanceService.deleteAllUserSelectPerformanceKeywordByUserId(user.getUserId());
			if ((!performanceKeyword.trim().equals("")) && (!performanceKeyword.isEmpty()) && (performanceKeyword != null)) {
				
				String[] performanceList = null;
				performanceList = performanceKeyword.trim().split(",");
				
				for (String s : performanceList) {
					s = s.replace(",", "");
					
					PerformanceKeywordDTO performanceKeywordDTO = new PerformanceKeywordDTO();
					
					performanceKeywordDTO.setGenre(s);
					performanceKeywordDTO.setUserId(user.getUserId());
					
					performanceService.savePerformanceKeyword(performanceKeywordDTO);
			...
			if ((!performanceKeyword.trim().equals("")) && (!performanceKeyword.isEmpty()) && (performanceKeyword != null)) {
				...
				ArrayList<String> allKeywords = new ArrayList<>(Arrays.asList(
						"연극", "무용", "대중무용", "서양음악(클래식)", "한국음악(국악)", "대중음악", "복합", "서커스/마술", "콘서트"
				));
				...
				for (String all : allKeywords){
					
					if (! list2.contains(all)) {
						PerformanceKeywordDTO performanceKeywordDTO = new PerformanceKeywordDTO();
						
						performanceKeywordDTO.setGenre(all);
						performanceKeywordDTO.setUserId(user.getUserId());
